...
| [\[chore\] Add signal requirement level to yaml and jinja templates (#340)](https://github.com/open-telemetry/semantic-conventions-genai/pull/340) | lmolkova |  | ✅ | ✅ | 17h |
| [\[chore\] Add basic agent.md (#342)](https://github.com/open-telemetry/semantic-conventions-genai/pull/342) | lmolkova |  | ✅ | ✅ | 15h |
| [semconv for a2a protocol (#195)](https://github.com/open-telemetry/semantic-conventions-genai/pull/195) | eternalcuriouslearner | aabmass<br>JWinermaSplunk<br>pwkowalski&nbsp;💬⁠✔️<br>trask | ✅ | ✅ | 11h |
...
| [Add json-schema annotation for complex attributes, render boilerplate note in templates (#330)](https://github.com/open-telemetry/semantic-conventions-genai/pull/330) | lmolkova | JWinermaSplunk<br>trask&nbsp;✅ | ✅ | ✅ | 51m |
...
| [Add experimental GenAI context selection event (#190)](https://github.com/open-telemetry/semantic-conventions-genai/pull/190) | caioribeiroclw-pixel | lmolkova&nbsp;🔴<br>trask | ❌ | ❌ | 16h |
...
| [Introduce `gen_ai.invoke_agent.{inference,tool}_calls` (#336)](https://github.com/open-telemetry/semantic-conventions-genai/pull/336) | RKest | aabmass&nbsp;💬⁠✅<br>lmolkova&nbsp;💬⁠✅ | ✅ | ✅ | 6h |
...
PR #330
llm: pr-conversation -> reviewer (The reviewer asked a question, and the author replied with the explanation/change; the ball is back with the reviewer to confirm or close the thread.)
...
PR #291
llm: PRRT_kwDOSUeMrM6MBE93 -> author (The bot flagged a concrete docstring mismatch that needs the PR author to update the reference.)
llm: PRRT_kwDOSUeMrM6MBE-U -> author (The latest comment is a review note asking to make the casing examples consistent, so the PR author needs to update the registry and respond.)
llm: PRRT_kwDOSUeMrM6MBE-i -> author (The latest bot review asks to add an example value for `gen_ai.agent.trust.score`, so the PR author needs to update the registry.)
llm: PRRT_kwDOSUeMrM6MBE-0 -> author (The latest comment is a bot review asking for an added example for `gen_ai.agent.drift.score`, so the PR author needs to make the change.)
llm: PRRT_kwDOSUeMrM6MBE_D -> author (The latest comment is a review bot suggestion to change the spec wording from unconditional opt-in to recommended when the score is set, so the PR author needs to act or respond.)
llm: PRRT_kwDOSUeMrM6MBE_W -> author (The last comment is a review suggestion from the bot asking for a spec change, so the PR author needs to update or जवाब/respond.)
llm: PRRT_kwDOSUeMrM6MBE_k -> author (The latest comment is a review suggestion from automation asking to change `scan.method` behavior, so the PR author needs to respond or update the spec.)
llm: pr-conversation -> author (The latest substantive comment is from a reviewer and asks for a change in how the method tokens are modeled, so the PR author needs to respond or adjust the implementation.)
...
llm: pr-conversation -> reviewer (The author says all requested fixes are addressed and explicitly asks the reviewer to take another look, with one open question about the unit tests.)
