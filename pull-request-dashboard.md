...
| [Add gen_ai.invoke_agent.duration and gen_ai.execute_tool.duration metrics (#201)](https://github.com/open-telemetry/semantic-conventions-genai/pull/201) | pvlsirotkin | lmolkova&nbsp;💬⁠✅<br>MikeGoldsmith&nbsp;🔴<br>trask | ✅ | ✅ | 4d |
...
| [gen-ai: add run guardrail span and security finding (#262)](https://github.com/open-telemetry/semantic-conventions-genai/pull/262) | nagkumar91 | aabmass<br>trask | ✅ | ✅ | 20h |
...
| [feat(gen-ai): add agent authorization observability attributes (#180) (#291)](https://github.com/open-telemetry/semantic-conventions-genai/pull/291) | thebenignhacker | 14m |
...
PR #195
llm: PRRT_kwDOSUeMrM6J16l4 -> author (A reviewer requested an additional change (“This should also include gRPC.”) and there is no follow-up reply yet, so the PR author needs to act.)
llm: PRRT_kwDOSUeMrM6J2BGg -> author (A reviewer asked whether the text should be changed to "subscribe" instead of "resubscribe", so the author needs to जवाब/adjust the PR.)
llm: PRRT_kwDOSUeMrM6J2dsF -> author (A reviewer suggested renaming the field, and there is no author reply yet, so the PR author needs to respond or make the change.)
llm: PRRT_kwDOSUeMrM6J2gX3 -> author (A reviewer suggested removing "authenticated" from the line, and the author has not replied yet.)
llm: PRRT_kwDOSUeMrM6J2wRS -> author (A reviewer suggested tightening the wording of "Associated" and implied the description should be updated; the author needs to respond or revise the PR.)
llm: PRRT_kwDOSUeMrM6J20Kt -> author (A reviewer suggested aligning the value with the A2A spec, and there is no author reply yet, so the PR author needs to respond or update the PR.)
llm: PRRT_kwDOSUeMrM6J29RI -> author (A reviewer requested a change (“Should include gRPC...”), and there’s no author reply yet, so the author needs to respond or update the PR.)
llm: PRRT_kwDOSUeMrM6J3J7f -> author (A reviewer asked for gRPC and JSON-RPC to be mentioned, and there’s no author reply yet, so the PR author needs to update the file or respond.)
llm: PRRT_kwDOSUeMrM6J25Z8 -> author (The last reviewer comment suggests deduplicating/removing the field but leaves uncertainty, so the author needs to respond with a decision or follow-up.)
llm: PRRT_kwDOSUeMrM6J3BZ8 -> author (A reviewer suggested making it opt-in here as well and did not close the topic, so the author needs to respond or make the change.)
llm: PRRT_kwDOSUeMrM6J60Lk -> author (A reviewer pointed out that `grpc` is not a valid `network.protocol.name` value and should be moved to `rpc.system.name`; the author needs to update the PR.)
