...
| [Add experimental GenAI context selection event (#190)](https://github.com/open-telemetry/semantic-conventions-genai/pull/190) | caioribeiroclw-pixel | trask | ❌ | ❌ | 6d |
...
| [Add GenAI client metrics to the anthropic reference scenario (#283)](https://github.com/open-telemetry/semantic-conventions-genai/pull/283) | AgentGymLeader | JWinermaSplunk<br>MikeGoldsmith&nbsp;🔴 | ✅ | ✅ | 22h |
| [Clarify that gen_ai.invoke_agent.duration is about in-proc / internal span (#321)](https://github.com/open-telemetry/semantic-conventions-genai/pull/321) | lmolkova | trask&nbsp;✅ | ✅ | ✅ | 21h |
| [Fix tool_call_response example field name (result -> response) (#323)](https://github.com/open-telemetry/semantic-conventions-genai/pull/323) | AgentGymLeader |  | ✅ | ✅ | 18h |
| [Add Agent Framework reference scenario (#325)](https://github.com/open-telemetry/semantic-conventions-genai/pull/325) | eavanvalkenburg |  | ✅ | ✅ | 6h |
| [Remove schema snapshot (#327)](https://github.com/open-telemetry/semantic-conventions-genai/pull/327) | lmolkova | trask&nbsp;✅ | ✅ | ✅ | 40m |
| [Update dependency google-adk to v2 (#328)](https://github.com/open-telemetry/semantic-conventions-genai/pull/328) | app/renovate |  | ❌ | ✅ | 34m |

## Waiting on authors

| PR | Author | Reviewers | CI | Conflicts | Age |
|---|---|---|:---:|:---:|:---:|
| [Add gen_ai.server.inter_token_latency metric (#164)](https://github.com/open-telemetry/semantic-conventions-genai/pull/164) | Jwrede | Cirilla-zmh<br>lmolkova&nbsp;💬<br>trask | ✅ | ❌ | 31d |
| [gen-ai: add evaluation operation name and gen_ai.evaluate.internal span (#185)](https://github.com/open-telemetry/semantic-conventions-genai/pull/185) | hippoley | Cirilla-zmh&nbsp;💬<br>singankit&nbsp;💬 | ❌ | ✅ | 29d |
| [gen-ai: add gen_ai.response.id to deepeval evaluation result event (#184)](https://github.com/open-telemetry/semantic-conventions-genai/pull/184) | hippoley | lmolkova&nbsp;✅<br>MikeGoldsmith&nbsp;🔴<br>trask | ✅ | ✅ | 22d |
| [Add modality, cache, and phase breakdowns for token usage (#197)](https://github.com/open-telemetry/semantic-conventions-genai/pull/197) | trask | alexmojaki&nbsp;💬<br>lmolkova&nbsp;💬<br>Nik-Reddy&nbsp;💬 | ✅ | ❌ | 19d |
...
| [Add gen_ai.invoke_agent.server span (SERVER kind) (#252)](https://github.com/open-telemetry/semantic-conventions-genai/pull/252) | singankit | Cirilla-zmh&nbsp;💬<br>trask | ✅ | ❌ | 14d |
...
| [Add gen_ai.agent.steps metric (#203)](https://github.com/open-telemetry/semantic-conventions-genai/pull/203) | pvlsirotkin | aabmass&nbsp;✅<br>lmolkova&nbsp;💬<br>MikeGoldsmith&nbsp;🔴<br>trask | ✅ | ❌ | 8d |
| [Add gen_ai.agent.input.content.size and gen_ai.agent.output.content.size metrics (#202)](https://github.com/open-telemetry/semantic-conventions-genai/pull/202) | pvlsirotkin | lmolkova&nbsp;💬<br>MikeGoldsmith&nbsp;🔴<br>trask | ✅ | ❌ | 8d |
| [chore: auto-regenerate outputs on SEMCONV_VERSION bumps via Renovate post-upgrade task (#290)](https://github.com/open-telemetry/semantic-conventions-genai/pull/290) | lmolkova | Copilot<br>lmolkova&nbsp;✔️<br>trask&nbsp;💬 | ✅ | ✅ | 5d |
...
| [semconv for a2a protocol (#195)](https://github.com/open-telemetry/semantic-conventions-genai/pull/195) | eternalcuriouslearner | aabmass<br>JWinermaSplunk<br>pwkowalski&nbsp;💬<br>trask | ✅ | ✅ | 1d |
| [Add prompt versioning and variable support to GenAI attributes (#179)](https://github.com/open-telemetry/semantic-conventions-genai/pull/179) | steverao | alexmojaki&nbsp;💬⁠✅<br>lmolkova&nbsp;✅<br>trask | ✅ | ✅ | 9h |
| [Migrate anthropic reference scenario to opentelemetry-util-genai (#324)](https://github.com/open-telemetry/semantic-conventions-genai/pull/324) | AgentGymLeader |  | ✅ | ✅ | 4h |
| [Align attributes between invoke_agent.internal and execute_tool spans and metrics (#322)](https://github.com/open-telemetry/semantic-conventions-genai/pull/322) | lmolkova |  | ✅ | ✅ | 43m |
...
| [Fix `tool_call_response` field name: `result` → `response` in examples (#326)](https://github.com/open-telemetry/semantic-conventions-genai/pull/326) | app/copilot-swe-agent | 34m |
| [Generalize review guidance comment (#329)](https://github.com/open-telemetry/semantic-conventions-genai/pull/329) | trask | 29m |
...
```text
PR #324
llm: PRRT_kwDOSUeMrM6K4OHc -> author (The latest comment is a bot review note flagging missing GenAI metrics coverage, so the PR author needs to update the metric list or respond to the issue.)
llm: PRRT_kwDOSUeMrM6K4OHx -> author (The bot flagged a concrete inconsistency in the scenario output versus the failing SDK call, so the PR author needs to update the scenario.)

PR #322
llm: PRRT_kwDOSUeMrM6Kth7r -> reviewer (The author has replied with the rationale for the metric removal, so the ball is back with the reviewer to confirm or continue the review.)
llm: PRRT_kwDOSUeMrM6Ktirm -> reviewer (The latest comment is from the author, so the ball is with the reviewer to respond or acknowledge the point.)
llm: PRRT_kwDOSUeMrM6K6k6e -> author (The bot flagged a required code change: these attributes should be set at span creation time, so the PR author needs to update the scenario.)
llm: PRRT_kwDOSUeMrM6K6k6l -> author (The latest comment is a review note from automation requesting a change to set sampling-relevant attributes at span creation time, so the PR author needs to update the scenario.)
llm: PRRT_kwDOSUeMrM6K6k6x -> author (The bot reviewer raised a concrete code change request, and no author reply appears in the thread yet.)
llm: PRRT_kwDOSUeMrM6K6k6- -> author (The latest comment is a review bot requesting a changelog edit, so the PR author needs to update the fragment.)
llm: PRRT_kwDOSUeMrM6K6k7M -> author (The latest comment is a bot review note requesting a cleanup in `model/gen-ai/spans.yaml`, so the PR author needs to make the change or जवाब back.)
llm: PRRT_kwDOSUeMrM6K6k7R -> author (The bot requested reference scenarios to be updated so sampling-relevant tool attributes are passed at span creation time; that points to PR changes still needed from the author.)
...
PR #195
llm: PRRT_kwDOSUeMrM6KRsU9 -> author (Reviewer asked for a clarification and possible model change; the author needs to respond or update the PR.)
...
PR #179
llm: PRRT_kwDOSUeMrM6Kz7CF -> author (A reviewer left a substantive note about a deprecated feature, so the author should respond or adjust the PR.)

PR #165
llm: PRRT_kwDOSUeMrM6KdZA7 -> author (The last comment is a bot review requesting changelog and reference-scenario updates, so the PR author needs to act.)
llm: PRRT_kwDOSUeMrM6KdZBh -> author (The latest comment is a bot review request asking to update the PR description or committed artifacts, so the PR author needs to act.)
