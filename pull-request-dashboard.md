...
| [Add json-schema annotation for complex attributes, render boilerplate note in templates (#330)](https://github.com/open-telemetry/semantic-conventions-genai/pull/330) | lmolkova | trask&nbsp;✅ | ✅ | ✅ | 6h |
| [Add Agent Framework reference scenario (#325)](https://github.com/open-telemetry/semantic-conventions-genai/pull/325) | eavanvalkenburg | lmolkova&nbsp;💬 | ✅ | ✅ | 23m |
...
| [Align attributes between invoke_agent.internal and execute_tool spans and metrics (#322)](https://github.com/open-telemetry/semantic-conventions-genai/pull/322) | lmolkova | trask&nbsp;✅ | ✅ | ✅ | 10h |
...
PR #325
llm: PRRT_kwDOSUeMrM6Lcv_P -> reviewer (The author replied to the reviewer’s request and explained the workflow-span work is backlog-related, so the next response is on the reviewer/maintainer side.)
llm: PRRT_kwDOSUeMrM6LcvBV -> reviewer (The author answered the reviewer’s question about why inference spans are created, so the ball is back with the reviewer to acknowledge or continue the review.)
...
PR #179
llm: PRRT_kwDOSUeMrM6Kz7CF -> author (A reviewer noted that OpenAI has deprecated the prompt template feature, so the author needs to respond by updating or acknowledging the affected scenario.)
