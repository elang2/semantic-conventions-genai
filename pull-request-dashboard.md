...
| [gen-ai: make input-messages BlobPart content optional and add stripped_reason (#144)](https://github.com/open-telemetry/semantic-conventions-genai/pull/144) | Mandark-droid | lmolkova<br>trask | ✅ | ✅ | 3d |
...
| [Add experimental GenAI context selection event (#190)](https://github.com/open-telemetry/semantic-conventions-genai/pull/190) | caioribeiroclw-pixel | trask | ❌ | ❌ | 20h |
| [Add GenAI client metrics to the anthropic reference scenario (#283)](https://github.com/open-telemetry/semantic-conventions-genai/pull/283) | AgentGymLeader |  | ✅ | ✅ | 4h |
| [Remove gen_ai.provider.name from internal agent spans (#289)](https://github.com/open-telemetry/semantic-conventions-genai/pull/289) | lmolkova |  | ✅ | ❌ | 2h |
| [Add `gen_ai.request.reasoning.level` attribute (#258)](https://github.com/open-telemetry/semantic-conventions-genai/pull/258) | katsuhisa91 | JWinermaSplunk&nbsp;✅<br>lmolkova<br>trask | ✅ | ✅ | 39m |

## Waiting on authors

| PR | Author | Reviewers | CI | Conflicts | Age |
|---|---|---|:---:|:---:|:---:|
| [Add gen_ai.server.inter_token_latency metric (#164)](https://github.com/open-telemetry/semantic-conventions-genai/pull/164) | Jwrede | Cirilla-zmh<br>lmolkova&nbsp;💬<br>trask | ✅ | ❌ | 26d |
...
| [Add modality, cache, and phase breakdowns for token usage (#197)](https://github.com/open-telemetry/semantic-conventions-genai/pull/197) | trask | alexmojaki&nbsp;💬<br>lmolkova&nbsp;💬<br>Nik-Reddy&nbsp;💬 | ✅ | ❌ | 14d |
| [Add gen_ai.invoke_agent.server span (SERVER kind) (#252)](https://github.com/open-telemetry/semantic-conventions-genai/pull/252) | singankit | Cirilla-zmh&nbsp;💬<br>Copilot&nbsp;💬<br>trask | ✅ | ❌ | 9d |
| [Add prompt versioning and variable support to GenAI attributes (#179)](https://github.com/open-telemetry/semantic-conventions-genai/pull/179) | steverao | lmolkova&nbsp;💬<br>trask | ✅ | ❌ | 9d |
...
| [Add gen_ai.agent.invocation.id attribute for invoke_agent spans (#250)](https://github.com/open-telemetry/semantic-conventions-genai/pull/250) | singankit | lmolkova&nbsp;💬<br>MikeGoldsmith&nbsp;🔴<br>trask | ✅ | ❌ | 3d |
| [Clarify scope of `gen_ai.client.operation.duration` metric (#215)](https://github.com/open-telemetry/semantic-conventions-genai/pull/215) | trask | lmolkova | ✅ | ❌ | 3d |
...
| [gen-ai: add run guardrail span and security finding (#262)](https://github.com/open-telemetry/semantic-conventions-genai/pull/262) | nagkumar91 | aabmass<br>trask | ✅ | ✅ | 2d |
...
| [chore: auto-regenerate outputs on SEMCONV_VERSION bumps via Renovate post-upgrade task (#290)](https://github.com/open-telemetry/semantic-conventions-genai/pull/290) | web-flow | Copilot<br>Copilot&nbsp;💬<br>lmolkova&nbsp;✅ | ✅ | ✅ | 2h |
...
| PR | Author | Updated |
|---|---|:---:|
| [proposal: agent.threat.detection.* attributes + event (closes #132) (#165)](https://github.com/open-telemetry/semantic-conventions-genai/pull/165) | eeee2345 | 22d |
...
PR #289
llm: PRRT_kwDOSUeMrM6JXu8q -> reviewer (The latest comment is from the PR author and appears to be a substantive note rather than a closure, so the reviewer/maintainer needs to respond or review next.)

PR #270
llm: PRRT_kwDOSUeMrM6I6cGM -> author (The last reviewer comment clarifies the intended split between hosted and local agents, so the ball is back with the PR author to respond and decide how to adjust the design.)
...
PR #238
llm: PRRT_kwDOSUeMrM6JNla8 -> author (A reviewer asked whether any of the listed reasons mark the span as an error and set `error.type`, so the author needs to जवाब/clarify.)
llm: PRRT_kwDOSUeMrM6JN_PC -> author (The reviewer asked for a change to derive `is` from the actual response, and there is no author follow-up yet.)
llm: PRRT_kwDOSUeMrM6JOJhC -> author (A reviewer said the span should use `error.type` instead of finish reason, so the PR author needs to update the scenario or respond.)
llm: PRRT_kwDOSUeMrM6JOKFj -> author (A reviewer pointed out a likely missing `error.type` on the `invoke_agent` span, and there’s no author reply yet.)
llm: PRRT_kwDOSUeMrM6JOiLW -> author (The latest comment is from a reviewer raising a terminology concern and providing alternatives, so the author needs to respond or revise the PR.)
llm: pr-conversation -> author (The reviewer requested changes and proposed a clearer attribute split, so the PR author needs to respond or update the PR.)
