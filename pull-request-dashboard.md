...
| [Add json-schema annotation for complex attributes, render boilerplate note in templates (#330)](https://github.com/open-telemetry/semantic-conventions-genai/pull/330) | lmolkova | trask&nbsp;✅ | ✅ | ✅ | 2h |
...
| [Align attributes between invoke_agent.internal and execute_tool spans and metrics (#322)](https://github.com/open-telemetry/semantic-conventions-genai/pull/322) | lmolkova | trask&nbsp;✅ | ✅ | ✅ | 6h |
| [Add Agent Framework reference scenario (#325)](https://github.com/open-telemetry/semantic-conventions-genai/pull/325) | eavanvalkenburg | lmolkova&nbsp;💬 | ✅ | ✅ | 35m |
...
```text
PR #330
llm: PRRT_kwDOSUeMrM6K7L7y -> none (The only comment is a reviewer/approver reaction emoji with no request or unresolved question, so the thread is effectively closed.)

PR #325
llm: PRRT_kwDOSUeMrM6LcvBV -> author (A reviewer asked a substantive question and no one has პასუხed yet, so the PR author needs to respond.)
llm: PRRT_kwDOSUeMrM6Lcv_P -> author (Reviewer suggested producing a workflow span here, so the author needs to update the PR.)

PR #322
llm: PRRT_kwDOSUeMrM6Kth7r -> reviewer (The only comment is from the author explaining the metric removal, so the ball is with a reviewer to review or confirm the change.)
llm: PRRT_kwDOSUeMrM6Ktirm -> reviewer (The author’s last comment explains the issue and ties it to the PR, so the ball is back with the reviewer to respond or confirm.)
llm: pr-conversation -> author (Reviewer requested an additional reference scenario/fixture to prove stable agent/session identity across spans and metrics, so the PR author needs to implement or respond.)
...
llm: pr-conversation -> author (The latest comment is a reviewer suggestion asking for spec changes and test coverage, so the PR author needs to respond or update the PR.)
...
PR #179
llm: PRRT_kwDOSUeMrM6Kz7CF -> author (A reviewer noted that OpenAI has deprecated this prompt template feature, so the author needs to respond by updating or acknowledging the thread.)
llm: pr-conversation -> author (A reviewer asked the PR author to rebase and resolve conflicts, so the author needs to act next.)
