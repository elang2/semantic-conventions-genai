...
invocation. An iteration is one pass through the agent's control
loop: a model request together with any resulting tool executions.

Instrumentations SHOULD record the framework's configured value
verbatim, without normalization. Exact bounds may differ by one
cycle across frameworks (for example, a tool-iteration cap may
