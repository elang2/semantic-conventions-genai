...
| [`gen_ai.usage.cost.amount`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Conditionally Required` [13] | double | The monetary cost of a single GenAI inference operation. [14] | `0.0023`; `0.015`; `0.42` |
| [`gen_ai.usage.cost.currency`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Conditionally Required` If `gen_ai.usage.cost.amount` is set. | string | ISO 4217 currency code for the cost value. [15] | `USD`; `EUR`; `GBP` |
| [`gen_ai.usage.cost.source`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Conditionally Required` If `gen_ai.usage.cost.amount` is set. | string | The source of the cost value. [16] | `provider`; `local` |
| [`server.port`](https://github.com/open-telemetry/semantic-conventions/blob/v1.44.0/docs/registry/attributes/server.md) | ![Stable](https://img.shields.io/badge/-stable-lightgreen) | `Conditionally Required` If `server.address` is set. | int | GenAI server port. [17] | `80`; `8080`; `443` |
...
**[13] `gen_ai.usage.cost.amount`:** If the response the client received contains a cost value and a currency can be determined for it as described on `gen_ai.usage.cost.currency`.

**[14] `gen_ai.usage.cost.amount`:** The cost in the currency specified by `gen_ai.usage.cost.currency`.
...
**[15] `gen_ai.usage.cost.currency`:** MUST be a three-letter ISO 4217 alphabetic code (e.g. "USD", "EUR").
...
**[16] `gen_ai.usage.cost.source`:** Distinguishes cost values received in the response from cost values
...
| Value | Description | Stability |
| --- | --- | --- |
| `local` | The emitting component computed the value itself. [48] | ![Development](https://img.shields.io/badge/-development-blue) |
| `provider` | The cost value was present in the response the client received. [49] | ![Development](https://img.shields.io/badge/-development-blue) |

**[48]:** No cost was present in the response; the emitting component
...
**[49]:** The cost value was present in the response the instrumented
